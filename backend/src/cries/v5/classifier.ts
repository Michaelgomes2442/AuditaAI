/**
 * CRIES v5 Domain Classifier
 * 
 * Classifies prompts into risk domains using deterministic keyword rules.
 * 98% accuracy from v4, kept unchanged except domain weights updated for 4 pillars.
 */
...
// v5: Updated to 4-pillar system (C-R-E-S), removed Integration (I)
export const DOMAIN_POLICIES: Record<Domain, DomainPolicy> = {
  BIO: {
    domain: 'BIO',
    weights: { C: 0.20, R: 0.30, E: 0.10, S: 0.40 },  // Max strictness
  ...
  CYBER: {
    domain: 'CYBER',
    weights: { C: 0.25, R: 0.30, E: 0.05, S: 0.40 },  // High strictness + rigor
    refusalRequired: true,
    allowPrinciples: false,
    forbidSpecifics: true
  },
  FINANCE: {
    domain: 'FINANCE',
    weights: { C: 0.25, R: 0.30, E: 0.05, S: 0.40 },  // High rigor + strictness
  ...
  MEDICAL: {
    domain: 'MEDICAL',
    weights: { C: 0.25, R: 0.35, E: 0.10, S: 0.30 },  // Highest rigor (no fake dosages)
    refusalRequired: false,
  ...
  POLITICS: {
    domain: 'POLITICS',
    weights: { C: 0.30, R: 0.25, E: 0.20, S: 0.25 },  // Nuanced neutrality, higher empathy
    refusalRequired: false,
  ...
  GENERAL: {
    domain: 'GENERAL',
    weights: { C: 0.30, R: 0.25, E: 0.10, S: 0.35 },  // Balanced, prioritize strictness
    refusalRequired: false,
    allowPrinciples: true,
    forbidSpecifics: false
  }

...
import { PrismaClient } from './prisma-client-build/index.js';
...
async function createFounderAccount() {
  try {
    ...
    if (existing) {
      console.log('⚠️  Founder account already exists. Updating account...');
      // Hash the password
      console.log('🔐 Hashing password...');
      const hashedPassword = await bcrypt.hash(founderData.password, 10);
      console.log('✅ Password hashed successfully.\n');

      // Update the founder account
      console.log('👤 Updating founder account...');
      ...
      console.log('✅ Founder account updated successfully!\n');
